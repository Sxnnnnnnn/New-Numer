...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/IntegrationGet/Trapezoidal", {}).then(
            (response) => {
    ...
    function Calculate() {
        ...
        let result = { I: i.toFixed(3), Error: error.toFixed(5) }
        data.push(result);
        setshowresult("yes")
    ...
    return (
        <>
            ...
            <div>
                <a href='/trapezoidal' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
