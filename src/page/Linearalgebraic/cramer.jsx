...
import { clone, column, det, evaluate } from "mathjs";
import Axios from "axios";

function App() {
    let [inputrow, setinputrow] = useState("");
    let [inputcolumn, setinputcolumn] = useState("");
    ...
    function savedata() {
        let save = { inputcolumn: inputcolumn, inputrow: inputrow, matrixa: matrixa, matrixb: matrixb };
    ...
    function getdata() {
        Axios.get("http://localhost:3001/linearGet/Cramer", {}).then(
            (response) => {
    ...
    function selectdata(item) {
        ...
        inputrow = temp.inputrow;
        inputcolumn = temp.inputcolumn;
    ...
    function Calculate() {
        ...
        for (let i = 0; i < parseInt(inputrow); i++) {
            let copya = clone(matrixa);
            for (let j = 0; j < parseInt(inputcolumn); j++) {
                copya[j][i] = matrixb[j]
            }
            let detai = det(copya)
            data.push(detai / deta)
        }
    ...
    return (
        <>
                ...
                <>
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 ">
                        <div>
                            ...
                            <input value={inputrow} onChange={(e) => setinputrow(parseInt(e.target.value))} class="w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500" type='number'>
                            </input>
                        </div>
                        <div>
                            <label className="text-3xl fold-bold text-slate-950">
                            ...
                            <input value={inputcolumn} onChange={(e) => setinputcolumn(parseInt(e.target.value))} class="w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500" type='number'>
                            </input>
                        </div>
            ...
            <div>
                <a href='/cramer' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
