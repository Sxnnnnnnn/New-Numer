...
import { evaluate, add, sqrt, clone, lusolve } from "mathjs"
...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/linearGet/Ludecomposition", {}).then(
            (response) => {
    ...
    function Calculate() {
        let n = row
        let x = lusolve(matrixa, matrixb)
        for (let i = 0; i < n; i++) {
            data.push(parseFloat(x[i][0]).toFixed(6))
        }
        console.log(data)
        setshowresult("yes")
    }
    ...
    return (
        <>
            {showresult == "inputn" ? (
                <>
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 ">
                        <div>
                            ...
                            <input value={row} onChange={(e) => setrow(parseInt(e.target.value))} class="w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500" type='number'>
                            </input>
                        </div>
                        <div>
                            <label className="text-3xl fold-bold text-slate-950">
                            ...
                            <input value={column} onChange={(e) => setcolumn(parseInt(e.target.value))} class="w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500" type='number'>
                            </input>
                        </div>
                ...
                <>
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 ">
                        ...
                        {data.map((item, index) => (
                            <h5 className="text-2xl font-bold text-zinc-900">
                                X  {index + 1} = {item}
                            </h5>
                        ))}
                    </div>
            ...
            <div>
                <a href='/ludecomposition' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
