...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/DifferentiationGet/Central", {}).then(
            (response) => {
    ...
    function Calculate() {
        ...
        if (choose == "First") {
            fpxi = (plus[1] - minus[1]) / (2 * h)
            diff = derivative(inputFx, "x").toString()

        } else if (choose == "Second") {
            fpxi = (plus[1] - (2 * plus[0]) + minus[1]) / (h ** 2)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Third") {
            fpxi = (plus[2] - (2 * plus[1]) + (2 * minus[1]) - minus[2]) / (2 * (h ** 3))
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Fourth") {
            fpxi = (plus[2] - (4 * plus[1]) + (6 * plus[0]) - (4 * minus[1]) + minus[2]) / (h ** 4)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
    ...
    return (
        <>
            ...
            <div>
                <a href='/central' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
