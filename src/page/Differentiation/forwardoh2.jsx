...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/DifferentiationGet/Forwardoh2", {}).then(
            (response) => {
    ...
    function Calculate() {
        ...
        if (choose == "First") {
            fpxi = (-fxi[2] + (4 * fxi[1]) - (3 * fxi[0])) / (2 * h)
            diff = derivative(inputFx, "x").toString()

        } else if (choose == "Second") {
            fpxi = (-fxi[3] + (4 * fxi[2]) - (5 * fxi[1]) + (2 * fxi[0])) / (h ** 2)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Third") {
            fpxi = ((-3 * fxi[4]) + (14 * fxi[3]) - (24 * fxi[2]) + (18 * fxi[1]) - (5 * fxi[0])) / (2 * (h ** 3))
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Fourth") {
            fpxi = ((-2 * fxi[5]) + (11 * fxi[4]) - (24 * fxi[3]) + (26 * fxi[2]) - (14 * fxi[1]) + (3 * fxi[0])) / (h ** 4)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
        ...
        err = diffture - fpxi
        let Result = { appox: fpxi.toFixed(5), truevule: diffture.toFixed(5), err: err.toFixed(5), X: xi.toFixed(5) }
        data.push(Result)
        setshowresult("Yes");
    }
    function check() {
    ...
    return (
        <>
            ...
            <div>
                <a href='/forwardoh2' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
