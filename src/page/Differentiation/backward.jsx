...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/DifferentiationGet/Backward", {}).then(
            (response) => {
    ...
    function Calculate() {
        ...
        if (choose == "First") {
            ...
            fpxi = (fxi[0] - (2 * fxi[1]) + fxi[2]) / (h ** 2)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Third") {
            fpxi = (fxi[0] - (3 * fxi[1]) + (3 * fxi[2]) - fxi[3]) / (h ** 3)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()

        } else if (choose == "Fourth") {
            fpxi = (fxi[0] - (4 * fxi[1]) + (6 * fxi[2]) - (4 * fxi[3]) + fxi[4]) / (h ** 4)
            diff = derivative(inputFx, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
            diff = derivative(diff, "x").toString()
    ...
    return (
        <>
            ...
            <div>
                <a href='/backward' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
