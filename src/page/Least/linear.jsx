...
function App() {
    ...
    function getdata() {
        Axios.get("http://localhost:3001/leastsquaresGet/Linear", {}).then(
            (response) => {
    ...
    function Calculate() {
        ...
        let xi, yi, xi2, xiyi, xxi;
        let sigmaxi = 0, sigmayi = 0, sigmaxi2 = 0, sigmaxy = 0;
        for (let i = 0; i < n; i++) {
            xi = parseFloat(x[i])
            sigmaxi = sigmaxi + xi
            xi2 = parseFloat(x[i]) ** 2
            sigmaxi2 = sigmaxi2 + xi2
            yi = parseFloat(y[i])
            sigmayi = sigmayi + yi
            xiyi = xi * yi
            sigmaxy = sigmaxy + xiyi
            xxi = { xi: xi.toFixed(5), yi: yi.toFixed(5), xi2: xi2.toFixed(5), xiyi: xiyi.toFixed(5) }
            box.push(xxi)
        }
        let matrixa = [[n, sigmaxi], [sigmaxi, sigmaxi2]], matrixb = [sigmayi, sigmaxy];
        ...
        let func = a[0].toFixed(5) + "+" + a[1].toFixed(5) + "*x";
        let fx = evaluate(func, { x: parseFloat(inputx) })
        let err = 0;
        for (let i = 0; i < n; i++) {
            err += (parseFloat(y[i]) - ((a[0]) + (a[1] * parseFloat(x[i])))) ** 2
        }
    ...
    return (
        <>
                ...
                <>
                    ...
                    <div class="grid grid-cols-1 gap-x-8 gap-y-6 ">
                        <p>
                            ...
                            <input type='number' onChange={(n) => setinputx(n.target.value)} className='w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500' >
                            </input>
                        </p>
                        <p>
                            <label className="text-3xl fold-bold text-slate-950">
                            ...
                            <input type='number' onChange={(n) => setinputn(n.target.value)} className='w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500' >
                            </input>
                        </p>
                        <div>
                        ...
                        <table className="table center">
                            ...
                            <tbody>
                                {x.map((item, index) => (
                                    <tr key={index}>
                                        <td>
                                            <input type='number' onChange={(e) => { x[index] = e.target.value }} className='w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500'>
                                            </input>
                                        </td>
                                        <td>
                                            <input type='number' onChange={(e) => { y[index] = e.target.value }} className='w-250  p-1 border rounded-md outline-none focus:ring-2 focus:ring-orange-500'>
                                            </input>
                                        </td>
                                    </tr>
                                ))}
            ...
            <div>
                <a href='/linear' className='text-gray-900 bg-gradient-to-r from-teal-200 to-lime-200 hover:bg-gradient-to-l hover:from-teal-200 hover:to-lime-200 focus:ring-4 focus:outline-none focus:ring-lime-200 dark:focus:ring-teal-700 font-medium rounded-lg text-sm px-5 py-2.5 text-center mr-2 mb-2'>Black</a>
            </div>
        </>
    )
}
